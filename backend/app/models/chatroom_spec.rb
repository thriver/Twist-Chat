RSpec.describe Chatroom do
  ...
  describe "When it is not a twist chat" do
    let(:normal_chat) {Chatroom.new(user: user, name: "chatroom")}
    it "should get 0 messages" do
      expect(normal_chat.get_messages).to eq []
    end

    it "should get 1 messages" do
      message = UserMessage.create(chatroom: normal_chat, user: user)
      normal_chat.reload
    ...
    it "should get 2 messages ordered by time of creation (DESC)" do
      message = UserMessage.create(chatroom: normal_chat, user: user)
      message2 = UserMessage.create(chatroom: normal_chat, user: user)
      normal_chat.reload
  ...
  describe "When it is a twist chat" do
    let(:twist_chat) {Chatroom.new(user: user, name: "chatroom", prompt: "prompt")}
    ...
    it "should get 1 messages" do
      message = UserMessage.create(chatroom: twist_chat, user: user)
    ...
    it "should get 2 messages ordered by time of creation (DESC)" do
      message = UserMessage.create(chatroom: twist_chat, user: user)
      message2 = UserMessage.create(chatroom: twist_chat, user: user)
      twistmessage = TwistMessage.create(user_message: message, message: "content")
