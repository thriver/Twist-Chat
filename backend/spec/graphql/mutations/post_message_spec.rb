  ...
  RSpec.describe PostMessage, type: :request do
    ...
    describe '.resolve' do
      it 'should add a new message if chatroom and user exist' do
        ...
        chatroom = Chatroom.create(user:, name: 'default_chat')
        content = 'message'
        expect do
          post '/graphql', params: { query: query(username: user.username, chatroom: chatroom.id, content:) }
        end.to change { UserMessage.count }.by(1)
        expect(UserMessage.where(user_id: user.id)
                          .where(chatroom_id: chatroom.id).where(content:).count).to be(1)
      end
      ...
      it 'should throw an error if user does not exist' do
        ...
        content = 'message'
        expect do
          post '/graphql', params: { query: query(username: random_username, chatroom: chatroom.id, content:) }
      ...
      it 'should throw an error if chatroom does not exist' do
        ...
        chatroom_id = -1
        expect do
          post '/graphql', params: { query: query(username: user.username, chatroom: chatroom_id, content:) }
        end.to change { UserMessage.count }.by(0)
        json = JSON.parse(response.body)
        expect(json['data']['postMessage']['errors']).to eq [
          {
