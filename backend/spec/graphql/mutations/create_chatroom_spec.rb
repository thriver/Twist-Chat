    ...
    describe ".resolve" do
      it "should add a new chatroom if the chatroom does not already exist" do
        ...
        expect(Chatroom.where(:user_id => user.id)
                          .where(:name => name).where(:prompt => twist).count).to be(1)
      end
      ...
      it "should not add new chatroom if the chatroom has no valid owner" do
        ...
        twist = "twist"
        expect do
          post '/graphql', params: {query: query(username: fake_username, name: name, prompt: twist)}
        end.to change {Chatroom.count}.by(0)
      ...
      it "should not add new chatroom if the chatroom already exists" do
        ...
        name = "duplicate"
        twist = "twist"
        Chatroom.create(user: user, name:name, prompt: twist)
        expect do
          post '/graphql', params: {query: query(username: user.username, name: name, prompt: twist)}
        end.to change {Chatroom.count}.by(0)
        json = JSON.parse(response.body)
